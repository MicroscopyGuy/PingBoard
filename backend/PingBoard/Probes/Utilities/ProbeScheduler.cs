...
public class ProbeScheduler : IProbeScheduler
{
    ...
    public void EndIntervalTracking()
    {
        _timer.Stop();
        _waitMinusProbeTime =
            _estimatedWaitTimeInBetweenProbesMs
            - TimeSpan.FromMilliseconds(_timer.Elapsed.TotalMilliseconds);
    }
    ...
    private TimeSpan CalculateProbingDelay()
    {
        TimeSpan adjustedWaitBeforeNextProbe =
            _waitMinusProbeTime > _minimumWaitBeforeNextProbeMs
                ? _waitMinusProbeTime
    ...
    public async Task DelayProbingAsync()
    {
        ...
        long remainingMs,
            imprecisionBufferRemainingTimeMs = 0;
